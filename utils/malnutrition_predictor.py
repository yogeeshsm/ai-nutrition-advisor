...
class MalnutritionPredictor:
    ...
    def predict_risk(self, child_data, diet_data=None):
        """Compatibility wrapper for predict method"""
        age = child_data.get('age_months', 36)
        weight = child_data.get('weight_kg', 12.0)
        height = child_data.get('height_cm', 90.0)
        res = self.predict(age_months=age, weight_kg=weight, height_cm=height)
        res['overall_risk'] = res['risk_level']
        for key in ['underweight', 'stunting', 'wasting']:
            if key in res['predictions']:
                res['predictions'][key]['risk_level'] = res['predictions'][key].get('risk', res['risk_level'])
                res['predictions'][key]['probability'] = res['confidence']
        return res
