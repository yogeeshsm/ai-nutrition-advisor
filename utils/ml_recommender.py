...
class MealRecommendationSystem:
    ...
    def get_collaborative_recommendations(self, child_id, top_n=10):
        ...
        similar_child_ids = [str(child['child_id'] if isinstance(child, dict) else child[0]) for child in similar_children]
        p = '%s' if getattr(db, 'DB_TYPE', 'sqlite') == 'mysql' else '?'
        placeholders = ','.join([p for _ in similar_child_ids])
        
        query = f"""
            SELECT mp.plan_data as ingredients, mp.nutrition_score, mp.total_cost
            FROM meal_plans mp
            WHERE mp.nutrition_score >= 70
    ...
    def optimize_weekly_variety(self, child_id, days=7):
        ...
        p = '%s' if getattr(db, 'DB_TYPE', 'sqlite') == 'mysql' else '?'
        placeholders = ','.join([p for _ in ingredient_names])
    ...
    def predict_ingredient_acceptance(self, child_id, ingredient_name):
        ...
        p = '%s' if getattr(db, 'DB_TYPE', 'sqlite') == 'mysql' else '?'
        for child_info in similar_children:
            similar_child_id = child_info['child_id'] if isinstance(child_info, dict) else child_info[0]
            query = f"""
                SELECT plan_data as ingredients, nutrition_score
                FROM meal_plans
                WHERE nutrition_score >= 60
            """
            try:
                plans = pd.read_sql_query(query, conn)
            except Exception:
                plans = pd.DataFrame()
        ...
        return min(1.0, max(0.0, acceptance_probability)), True
    ...
    def get_recommendation_explanation(self, child_id, ingredient_name):
        ...
        acceptance_res = self.predict_ingredient_acceptance(child_id, ingredient_name)
        acceptance = acceptance_res[0] if isinstance(acceptance_res, (tuple, list)) else acceptance_res
    ...
    def find_similar_children(self, child_id, top_n=5, n_neighbors=None):
        """
        Find children similar to the given child
        Returns list of dicts with child_id, similarity_score, etc.
        """
        ...
        try:
            ...
            target_query = f"""
                SELECT date_of_birth FROM children WHERE id = {p}
            """
