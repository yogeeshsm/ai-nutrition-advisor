...
class TestRecommender:
    ...
    def prepare_child_profile(self, child_id):
        ...
        if child_data.empty:
            conn.close()
            print(f'    [FAIL] Child data is empty!')
            return None
        ...
        print(f'    [OK] Profile created with {len(profile)} fields')
        return profile

print('\n[TEST] Testing prepare_child_profile in isolation...\n')
...
for child_id in [1, 2, 3]:
    ...
    if profile:
        print(f'  [OK] Success - age: {profile["age_years"]:.1f}')
    else:
        print(f'  [FAIL] Failed!')
    print()
